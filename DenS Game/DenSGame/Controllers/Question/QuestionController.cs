...
namespace DenSGame.Controllers.Question
{
    public class QuestionController : DenSGameController<DenSGameContext>
    {
        ...
        public JsonResult GetUserScores(long questionID)
        {
            ...
            var answersOfQuestion = _dataContext.Answers
                ...
                .Include(a_item => a_item.ChosenBy)
                .Where(a_item => a_item.Question.ID == question.ID)
                .OrderBy(a_item => a_item.User.Name)
                .ThenBy(a_item => a_item.AnswerString)
                .ToList();

            foreach(var answer in answersOfQuestion)
            {
                var pickedByDS = _dataContext.Answers
                    .Include(a_item => a_item.ChosenBy)
                    .Where(a_item => a_item.Question.ID == question.ID && 
                                    (a_item.PickedByDennis || a_item.PickedBySandra))
                    .ToList();
                var user = answer.User;
                answer.UserPickedDSAnswer = pickedByDS.Any(a_item => a_item.ChosenBy
                    .SingleOrDefault(a_user => a_user.ID == user.ID) != null);
            }
