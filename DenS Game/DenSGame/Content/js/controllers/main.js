densGame
    .controller("densGameClientController", function($rootScope, $scope, $http, $window, $timeout, growlService)
    {
        $scope.user = {};
        ...
        $scope.getUser = function(showGrowl)
        {
            $http.get("user/getuser")
                .success(function(result)
                {
                    ...
                    if ($scope.user.ID === 0)
                    {
                        growlService.growl("Welcome new 'Dennis & Sandra = 60' player", "inverse");
                    }
                    else
                    {
                        ...
                        else if ($scope.user.IsQuizmaster)
                        {
                            $rootScope.changeDashboard("selectQuestion");
                        }
                        else if ($scope.user.IsDennis || $scope.user.IsSandra)
                        {
        ...
        $rootScope.getCurrentQuestion = function()
        {
            $http.get("question/getcurrent")
                .success(function(result)
                {
                    ...
                    {
                        // Make sure Dennis and Sandra are not on this page anymore
                        if($scope.user.IsDennis || $scope.user.IsSandra)
                        {
                            $rootScope.changeDashboard("selectWinningAnswer");
                        }
                    }
    ...
    .controller("selectQuestionClientController", function($rootScope, $scope, $http, growlService)
    {
        ...
        $rootScope.getAllQuestions = function()
        {
            $http.get("marten/getallquestions")
                .success(function(result)
                {
                    $scope.runningQuestion = result.runningQuestion;
        ...
        $scope.changeQuestionStatus = function (questionId, status)
        {
            var postParams =
            {
            ...
            $http.post("marten/ChangeQuestionStatus", postParams)
                .success(function (result)
                {
                    $rootScope.getAllQuestions();
                });
    ...
    .controller("selectWinningAnswerClientController", function($rootScope, $scope, $http, growlService)
    {
        ...
        {
            $http.get("marten/getallanswers")
                .success(function(result)
                {
                    if (result.showingScores)
                    {
                        // Show the scores for Dennis and Sandra at the end of the round as well
                        $rootScope.changeDashboard("currentQuestion");
                    }
                    $scope.clientModel = result;
                });
        };
        ...
        {
            var url = "marten/";
            if ($scope.user.IsDennis)
            {
                url += "DennisSetsAnswer";
            }
            else if ($scope.user.IsSandra)
            {
                url += "SandraSetsAnswer";
            }

            var postParams =
            {
                answerID: answer.ID,
                questionID: $scope.clientModel.question.ID
            };
        ...
        {
            var selected = false;
            angular.forEach($scope.clientModel.answers, function (answer)
            {
                if (answer.Correct)
                {
                    selected = true;
        ...
        /*$scope.stopSubmittingAnswers = function()
        {
            ...
            var postParams =
            {
                id: $scope.clientModel.question.ID
            };
